...
void print_usage(const char* argv0, bool server) {
    ...
    fprintf(stderr,
        ...
        "      --target-faces N     QEM target faces (default: 300K @1024 / 150K @512)\n"
...
bool parse_args(int argc, char** argv, TrellisParams& p) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--target-faces" || a == "--target_faces")
                                                   { const char* v = need(a.c_str()); if (!v) return false; p.target_faces = atoi(v); }
